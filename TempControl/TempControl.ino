...
#include <OneWire.h>  // Base protocol for DS12B20 sensor
#include <DallasTemperature.h>

LiquidCrystal lcd(12, 11, 5, 4, 9, 10);  //lcd connected pins

//DS12B20 on D8 - requires 4.7k pull up to 5V
const byte ONE_WIRE_BUS = 8;  // Use D8
...
public:
  ...
  float getCelcius() {
    ...
    // getTempCByIndex returns -127on error; pass it through
    // and let the caller decide what to do (Heater wotn't trigger at -127)
    return t;
  }

  float getFahrenheit() {
    return 1.8f * getCelcius() + 32.0f;
  }
};
...
public:
  ...
  void checkTimeout() {
    ...
    if ((millis() - this->buttonTimer) > buttonTimeout) {
      ...
      for (int i = 0; i < 4; i++)
        EEPROM.update(i * sizeof(byte), settings[i]);
      EEPROM.update(4 * sizeof(byte), eepromSet);
    }
  }
  ...
  void showError(float c, bool b1, bool b2) {
    lcd.clear();
    lcd.setCursor(0, 0);
...
void setup() {
  ...
  warmer.init();
  disp.setButtons();
  attachInterrupt(digitalPinToInterrupt(2), onButton1Press, FALLING);
  attachInterrupt(digitalPinToInterrupt(3), onButton2Press, FALLING);
}

void loop() {
  float c = temp.getCelcius();
  ...
  if (c == DEVICE_DISCONNECTED_C) {
    warmer.turnOff();
    disp.showError(c, button1Pressed, button2Pressed);
  } else {
    if (c < onTemp) warmer.turnOn();
    else if (c > offTemp) warmer.turnOff();
    disp.show();
  }
}
